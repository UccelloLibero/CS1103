...
/**
This is a test program for the Tape class that creates a window containing
a graphical representation of the tape with an arrow pointing to
the current cell. There are buttons for moving the current cell to the left
and to the right. A text input box shows the content of the current cell.
This box can be edited, and there is a "Set" button that copies the contents
of the cell (actually just the first character) to the current cell.
*/
...
public class TestTapeGUI extends JPanel {
  ...
  public TestTapeGUI(String initialContent) {
    ...
    if (initialContent != null && initialContent.length() > 0) {
      ...
      // move back over the last character written
      tape.moveLeft();
    }

    ButtonListener listener = new ButtonListener();
    tapePanel = new TapePanel();
    tapePanel.setPrefferedSize(new Dimensions(500, 130));
