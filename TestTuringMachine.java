...
/**
This is a test program for teh TuringMachine class. It creates three machines
...
public class TestTuringMachine {
  ...
  public static void main(String[] args) {
    ...
    writeMachine.addRules(new Rule[] {
      // writes Hello on the tape then halts
      new Rule(0, " ", 1, "H", false),
      new Rule(0, " ", 2, "e", false),
      new Rule(0, " ", 3, "l", false),
      new Rule(0, " ", 4, "l", false),
      new Rule(0, " ", -1, "o", false)
    });
    ...
    badMachine.addRules(new Rule[] {
      // writes ERROR on the tape then fails
      new Rule(0, " ", 1, "R", true),
      new Rule(0, " ", 2, "O", true),
      new Rule(0, " ", 3, "R", true),
      new Rule(0, " ", 4, "R", true),
      new Rule(0, " ", 5, "E", true) // no rule for state 5
    });

    System.out.println("Running machine #2. Should throw an IllegalStateException.");
    try {
      badMachine.run(new Tape());
      System.out.println("No Error was thrown.");
    }
    catch (IllegalStateException e) {
      System.out.println("Caught an Illegal Argument Exception, with error message");
      System.out.println(e.getMessage());
    }

    // a string of letters a and b for input to the copy machine
    ...
    // now the input is written on tape, with the machine on the rightmost character
    tape.moveLeft();


    // copies a string of letters a and break;
    TuringMachine copyMachine = new TuringMachine();

    // machine must start on leftmost character in the string
    copyMachine.addRules(new Rule[] {
      // rules for copying a
      new Rule(0, "a", 1, "x", true),
      ...
      // rules for copying break;
      new Rule(0, "b", 4, "y", true),
      ...
      // rules that change x and y back to a and b and then halt
      new Rule(0, " ", 8, " ", false),
    ...
    System.out.println("nRunning machine #3. Output should be: " + input + " " + input);
