public class SimpleRandomSentences {
  // declare <conjunction> ::= and | or | but | because
  static final String[] conjunction = {"and", "or", "but", "because"};

  // declare <proper_noun> ::= Fred | Jane | Richard Nixon | Miss America
  static final String[] proper_noun = {"Fred", "Jane", "Richard Nixon", "Miss America"};

  // declare <common_noun> ::= man | woman | fish | elephant | unicorn
  static final String[] common_noun = {"man", "women", "fish", "elephant", "unicorn"};

  // declare <determiner> ::= a | the | every | some
  static final String[] determiner = {"a", "the", "every", "some"};

  // declare <adjective> ::= big | tiny | pretty | bald
  static final String[] adjective = {"big", "tiny", "pretty", "bald"};

  // declare <intransitive_verb> ::= runs | jumps | talks | sleeps
  static final String[] intransitive_verb = {"runs", "jumps", "talks", "sleeps"};

  // declare <transitive_verb> ::= loves | hates | sees | knows | looks for | finds
  ...
  static void randomNounPhrase() {
    if (Math.random() > 0.75) {
      ...
      if (Math.random() > 0.5) {
        System.out.print(" " + randomItem(adjective) + " " + randomItem(common_noun) + ".");
        // if (Math.random() > 0.5) {
