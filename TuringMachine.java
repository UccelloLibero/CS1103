...
/**
This class represents Turing Machine. A Turing Machine is a simple
computing device that moves back and forth along a Tape
(see {@link Tape}), reading and writing characters.
The machine has a state, which is represented by an integer. It also has a
program which consists of a list of rules (see {@link Rule}).
*/
...
public class TuringMachine {
  ...
  // @param rule A non-null rule to be added to the program.
  // A null value will not cuase an immediate error, but will produce NullPointExceptions
    ...
    public String run(Tape tape) throws IllegalStateException {
      ...
      while (currentState >= 0) {
        ...
        if (applicableRule == null) {
          throw new IllegalStateException("Cannot find an applicable rule: tape content = " + tape.getTapeContents());
          ...
          // System.out.println(applicableRule.currentState + " " + applicableRule.currentContent + " " + applicableRule.newState + " " + applicableRule.newContent + " " + applicableRule.moveLeft());
        }
      }
